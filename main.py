...
def motor_steering_dist(distance, speed, steer):
    ...
    while (abs(left_motor.angle() / 360 * wheelCircumference) < distance):
...
# shooter.run_angle(400, 280)
# time.sleep(1)

# motor_steering_dist(5,900,0)
# shooter.run_angle(400, 80)
...
if robot_number == 1:  
    while True:
        ...
        while gyro.angle() < 1080:
            print('heading', gyro.angle() )
            spinnn(200)
        # odometry()
        if state == "search":
            spinnn(200)
        ...
        if state == "chase":
            err = 217 - ball_x
            corr = GAIN * err
            motor_steering(400, corr)
            if counter == 1:
                if shooter_counter == 0:
                    shooter.run_angle(500, 290, wait = False)
                    shooter_counter == 1
                print("everything is working")
                    shooter.run_angle(500, 280, wait = False)
                    shooter_counter = 1
                counter = 2
            if ball_y == -1:
                state = "search"
            elif ball_y < 60:# to determine this through measurement of the camera FOV
                state = "capture"
            # continue
        elif state == "capture":
            if counter == 2:
            ...
            if shooter_counter == 1:
                ...
                time.sleep(1)
                motor_steering_dist(5, 900, 0)
                shooter.run_angle(500, 80, wait = True)
                print("heidi")
                shooter_counter = 0
            # if True:
            counter = 0
    ...
    while True:
        ...
        if state == "search":
            ...
            if ball_y != -1:
                ...
                motor_steering(0, 0)
                if counter == 0:
                    cage.run_angle(300, -120, wait=False)
                    counter = 1
                    if shooter_counter == 0:
                        shooter.run_angle(500, 280, wait = False)
                        shooter_counter = 1
        if state == "chase":
            ...
            if ball_y == -1:
                state = "search"
            elif ball_y < 35:# to determine this through measurement of the camera FOV
                state = "capture"
            # continue
        elif state == "capture":
            if counter == 1:
            ...
            if shooter_counter == 1:
                motor_steering_dist(5, 900, 0)
                shooter.run_angle(500, 80, wait = False)
            if shooter_counter == 11:
                shooter.run_angle(500, -110, wait = True)
                print("heidi")
                shooter_counter = 0
            counter = 0 
            motor_steering_dist(5, -700, 0)
            # if True:
            state = "search"
        prev_L = left_motor.angle()
        prev_R = right_motor.angle()
